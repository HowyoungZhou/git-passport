...
from gitu.config import Config, User
...
def get_user(name_or_alias) -> User:
...
def get_cur_user(file: str = 'local') -> (str, str):
    ...
    try:
        ...
        if ex.stderr is not None and ex.stderr != '':
            print('git: ' + ex.stderr.rstrip('\n'), file=sys.stderr)
...
def login(args):
    ...
    try:
        for k, v in user.get_git_configs().items():
            git_config.replace_all(k, str(v), args.git_conf_file)
        print('You are now logged in as %s <%s>.' % (user.name, user.email))
    except subprocess.CalledProcessError as ex:
        if ex.stderr is not None:
            print('git: ' + ex.stderr.rstrip('\n'), file=sys.stderr)
...
def register(args):
    ...
    config.add_user(User.from_dict(user))
    if args.alias is not None:
        check_name_conflict(args.alias, args.replace)
        config.append_alias(args.name, args.alias)
...
def edit(args):
    ...
    if args.new_name is not None:
        ...
        config.update_user_name(user.name, args.new_name)
    if args.alias is not None:
        check_name_conflict(args.alias, args.replace)
        config.append_alias(user.name, args.alias)
